...
class FeatureMetadataGenerator:
    ...
    def generate_feature_metadata(self) -> List[Dict[str, Any]]:
        ...
        for col_name, col_config in col_agg_dict.items():
            ...
            for condition_combo, agg_func, time_key in product(condition_combinations, aggregations, time_cond_list):
    ...
    def _generate_condition_combinations(
        self, conditions: List[bool], condition_lists: List[List[str]]
    ) -> List[Tuple[str, ...]]:
    ...
    def _generate_feature_name(
        self,
        team_name: str,
        feature_category: str,
        agg_func: str,
        condition_combo: Tuple[str, ...],
        col_name: str,
        time_key: str,
    ) -> str:
    ...
    def _parse_time_key_to_description(self, time_key: str) -> str:
        ...
        unit_map = {"h": "hour", "d": "day", "m": "month", "w": "week", "y": "year"}
    ...
    def _build_feature_cte(self, base_category: str, feature_metadata_list: List[Dict[str, Any]]) -> str:
        ...
        for col_name, col_config in col_agg_dict.items():
            ...
            for condition_combo, agg_func, time_key in product(condition_combinations, aggregations, time_cond_list):
                feature_name = self._generate_feature_name(
                    self.config["team_name"], self.config["feature_category"], agg_func, condition_combo, col_name, time_key
                )
    ...
    def validate_feature_names(self, batch_query: str) -> Dict[str, Any]:
        ...
        system_columns = {
            ...
            "is_ios",
            "login_id",
            "partition_date_unix_ts",
            "days_since_logintime",
            "l1h",
            "l6h",
            "l1d",
            "l3d",
        ...
        feature_names = [
            name for name in matches if name not in system_columns and any(name.startswith(agg) for agg in agg_functions)
        ]
