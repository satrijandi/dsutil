...
class TestTrainH2OAutoML:
    ...
    def test_train_h2o_automl_with_mlflow(self, mock_cpu_count, mock_automl_class, mock_h2o):
        ...
        result = train_h2o_automl(
            train_df, test_df, config, selected_features, mock_mlflow_manager
        )
    ...
    def test_train_h2o_automl_data_size_scaling(self, mock_automl_class, mock_h2o):
        ...
        small_train_df = pd.DataFrame({
            'feature1': range(5000),  # Small dataset < 10000
            'target': [0, 1] * 2500
        })
...
class TestEvaluateModel:
    ...
    def test_evaluate_model_with_mlflow(self, mock_roc_auc, mock_h2o):
        ...
        mock_leader.auc.return_value = 0.85
        mock_leader.varimp.return_value = pd.DataFrame({
            'variable': ['feature1'], 'relative_importance': [1.0]
        })
        ...
        train_df = pd.DataFrame({
            'feature1': [1], 'target': [0], 
            'date_col': pd.to_datetime(['2024-01-15'])
        })
        test_df = pd.DataFrame({
            'feature1': [2], 'target': [1], 
            'date_col': pd.to_datetime(['2024-02-15'])
        })
        ...
        with patch('tempfile.NamedTemporaryFile'), patch('os.unlink'):
            result = evaluate_model(
                mock_aml, train_df, test_df, config, selected_features, mock_mlflow_manager
            )
